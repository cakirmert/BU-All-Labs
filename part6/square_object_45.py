...
for position in positions:
    input(f"Place the square object in the {position} position (rotated 45°) and press Enter")
    measurements = []
    ...
    for _ in range(num_measurements):
        ...
        print(f"Measured Distance in {position} position (rotated 45°): {decoded_bytes} cm")
...
# Save results to CSV
with open("square_object_45_results.csv", "w", newline='') as f:
    writer = csv.writer(f)
