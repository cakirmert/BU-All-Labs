...
std_devs = []  # List to store standard deviations for each distance
...
# Read measurements and calculate mean durations and standard deviations
...
with open("measurement_results.csv", "w", newline='') as f:
    writer = csv.writer(f)
    writer.writerow(["Distance (cm)", "Mean Duration (µs)", "Standard Deviation (µs)"])
    for i, distance in enumerate(distances):
        writer.writerow([distance, mean_durations[i], std_devs[i]])

# Print mean durations and standard deviations for debugging
print("Distances (cm):", distances)
print("Mean Durations (µs):", mean_durations)
print("Standard Deviations (µs):", std_devs)

# Plot 2T(distance) with a linear trendline
plt.figure()  # Create a new figure
plt.errorbar(distances, mean_durations, yerr=std_devs, fmt='bo', label='Measured Data')  # Plot the measured data with error bars
